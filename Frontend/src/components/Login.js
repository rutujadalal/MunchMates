...
const Login = ({ setIsLoginOpen, onLoginSuccess }) => {
  ...
  const handleCheckMobile = async () => {
    try {
      setLoading(true);
      setError("");
      const response = await axios.post("http://localhost:5000/api/user/auth/check-mobile", { mobile });
  ...
  const handleSendOtp = async () => {
    try {
      setLoading(true);
      setError("");
      const response = await axios.post("http://localhost:5000/api/user/auth/send-otp", { mobile, name, email });
  ...
  const handleVerifyOtp = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/user/auth/verify-otp",
        { mobile, otp },
